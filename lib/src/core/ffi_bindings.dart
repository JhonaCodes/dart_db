...
typedef PostDataNative = Pointer<Utf8> Function(
    Pointer<Void> db, Pointer<Utf8> json);
...
typedef PutDataNative = Pointer<Utf8> Function(
    Pointer<Void> db, Pointer<Utf8> json);
...
typedef DeleteNative = Pointer<Utf8> Function(
    Pointer<Void> db, Pointer<Utf8> key);
...
class DbBindings {
  ...
  factory DbBindings.fromLibrary(DynamicLibrary lib) {
    ...
    final closeDb =
        lib.lookupFunction<CloseDbNative, CloseDb>('close_database');
    ...
    try {
      getAllKeys =
          lib.lookupFunction<GetAllKeysNative, GetAllKeys>('get_all_keys');
    } catch (e) {
      // Use dummy implementation
    ...
    try {
      freeString =
          lib.lookupFunction<FreeStringNative, FreeString>('free_string');
    } catch (e) {
